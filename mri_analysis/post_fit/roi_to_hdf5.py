...
To run:
>> cd to function
>> python post_fit/roi_to_hdf5.py [subject] [preproc] [registration]
-----------------------------------------------------------------------------------------
Exemple:
cd /home/mszinte/projects/PredictEye/mri_analysis/
python post_fit/roi_to_hdf5.py sub-01 fmriprep_dct_pca T1w
-----------------------------------------------------------------------------------------
...
for roi in rois:
    print('creating {roi} {preproc} {reg} h5 files (deriv, tc, tc_model)'.format(roi = roi, reg = regist_type, preproc = preproc))

    h5_file = "{h5_dir}/{roi}_{preproc}_{reg}.h5".format(h5_dir = h5_dir, roi = roi, reg = regist_type, preproc = preproc)
    ...
    deriv_file = "{deriv_dir}/{subject}_task-pRF_space-{reg}_{preproc}_deriv.nii.gz".format(deriv_dir = deriv_dir,subject = subject, reg = regist_type, preproc = preproc)
